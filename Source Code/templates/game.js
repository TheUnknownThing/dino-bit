...
Runner.config = {
  ...
  GAP_COEFFICIENT: 3.5, // former: 0.6
  GRAVITY: 0.5,
...
Runner.prototype = {
  ...
  update: function () {
    ...
    if (this.playing) {
      ...
      if (!collision) {
        ...
        var tmp = this.currentSpeed + this.config.ACCELERATION;
        if (tmp <= this.config.MAX_SPEED && tmp >= this.config.MIN_SPEED) {
  ...
  onKeyDown: function (e) {
    ...
    if (
      this.playing &&
      !this.crashed &&
      Runner.keycodes.ACCELERATE[e.keyCode]
    ) {
      e.preventDefault();
      var tmp = Runner.config.ACCELERATION + 0.01; // 控制加速度的上限阈值
      if (tmp < 0.1) Runner.config.ACCELERATION = tmp;
    }
    if (
      this.playing &&
      !this.crashed &&
      Runner.keycodes.STOPACCELERATE[e.keyCode]
    ) {
      e.preventDefault();
      var tmp = Runner.config.ACCELERATION - 0.02; // 控制加速度的下限阈值
      if (tmp > -0.1) Runner.config.ACCELERATION = tmp;
    }
