...
key={"1":["up","down","p","l"],"2":["q","a","i","o"],"3":["w","s","j","u"],"4":["space","d","k","m"]}
# isLow=[False]*4
if not ser.is_open:
    ser.open()
cnt=0
...
while True:
    ...
    if message:
        ...
        if message=="Con":print("ERROR!"); break
    ...
    for i in range(1,5):
        if islow[i]==0 and time.time()-lowtime[i]>0.011:
            keyboard.release(key[str(i)][1]) # BUG! : 多人小恐龙会存在串口传输的问题，控制每个小恐龙俯身的时间戳消除掉帧
            # if i==1:print("YES")