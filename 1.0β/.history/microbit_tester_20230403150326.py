...
while True:
    ...
    gesture = accerlerometer.current_gesture() # 判断当前microbit板的状态

    # 如果完全弯腰，即为腰彻底90度弯曲
    if gesture in ["face down", "down"]:
        jumping = True # 自己修改，此时有弯腰操作，小恐龙俯冲

    # 如果正在跳跃，则模拟跳跃动作
    if gesture == "freefall":
        jumping = True # 此时完全跳跃，小恐龙跳跃

        # 每次跳跃后，小恐龙有一段较短时间无法连续（再次）跳跃
        # 可能和while末尾的 sleep(50) 有关
